...
class enduranceView: UIViewController, UNUserNotificationCenterDelegate {
    ...
    var timerValue = 30 //change this value per the View
    
    func runTimer() {
        timer = Timer.scheduledTimer(timeInterval: 1, target: self,   selector: (#selector(enduranceView.updateTimer)), userInfo: nil, repeats: true)
        isTimerRunning = true
    }
    ...
    func willEnterForeground(noti: Notification) {
        if let y = UserDefaults.standard.object(forKey: "endTime") as? Date {
            let end = y
