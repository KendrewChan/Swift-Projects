...
class strengthView: UIViewController, UNUserNotificationCenterDelegate {
    ...
    var timerValue = 300 //change this value per the View
    
    func runTimer() {
        timer = Timer.scheduledTimer(timeInterval: 1, target: self,   selector: (#selector(strengthView.updateTimer)), userInfo: nil, repeats: true)
        isTimerRunning = true
    }
    ...
    func resetTimer() {
        ...
        seconds = timerValue
        timerLabel.text = timeString(time: TimeInterval(seconds))
        UserDefaults.standard.removeObject(forKey: "strTime")
    }
    ...
    func pauseWhenBackground(noti: Notification) {
        ...
        UserDefaults.standard.set(end, forKey: "strTime")
    }
    
    func willEnterForeground(noti: Notification) {
        if let y = UserDefaults.standard.object(forKey: "strTime") as? Date {
            let end = y
